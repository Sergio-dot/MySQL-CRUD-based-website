        ...
        <form action="../../includes/logic/hold.php" method="post">
            ...
            <div class="form-group">
                ...
                <select name="turno">
                    <br>
                    <?php
                    $result = visualizzaTurno();
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo '
                            <option value=' . $row["id_turno"] . '>' . $row["ora_inizio"] . ' - ' . $row['ora_fine'] . '</option>
                            ';
                        }
                    }
                    ?>
            ...
            <button type="submit" name="bt_conf_mod_cul" class="btn btn-primary mt-2">Conferma</button>
        </form>
    </div>
